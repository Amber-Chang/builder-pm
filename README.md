...
✅ **可安裝**。種子骨架 + 一鍵安裝腳本(`setup.sh`)已就緒,可選 **Claude Code、Codex 或兩者並用**;`template/` 就是會被裝進新專案的全部內容。
...
腳本會先問要使用 Claude Code、Codex 或兩者,再問專案名 / 技術棧 / 測試指令,把需要的模板複製到你指定的新資料夾、填好佔位符、`git init`,並詢問選用模組。
...
template/    # ★ 會被裝進新專案的全部內容(種子)
  ...
  AGENTS.md          # Codex 入口；轉接到 CLAUDE.md 共用治理
  .agents/skills/    # Codex 的 4 個角色 adapter
...
| Codex PR 審查 | Evaluator | Claude-only 可選用第二模型；Codex / 雙平台的正式 PR 必須啟用 `codex-pr-review` 並驗證 `pr-review-agent` |
